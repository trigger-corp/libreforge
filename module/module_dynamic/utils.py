...
def render_object(config, in_s):
    '''Render a Genshi template as an object

    :param config: data dictionary
    :param in_s: genshi template
    '''
    try:
        return ast.literal_eval(render_string(config, in_s))
    except:
        return None
...
def call_with_params(method, build_params, params):
...
# - Python 2 zipfile.extractall() does not restore permissions ----------------

from zipfile import ZipFile, ZipInfo

class BetterZipFile(ZipFile):
...
def value_for_platform(table):
    if sys.platform.startswith("win32"):
        ...
        return table["linux"]
    elif sys.platform in table:
        return table[sys.platform]
    else:
        raise AndroidSdkError("Unsupported platform '%s' for table '%s'" % (sys.platform, table))


# - stream wrapper unbuffered i/o ---------------------------------------------
