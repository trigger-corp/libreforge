...
class TestDefaultsAndOverridesAreSaved(object):
	...
	def test_deprecated_overrides_saved(self):
		...
		eq_(config.get("android.sdk"), "1")
		eq_(config["ios.profile.developer_certificate"], "2")
	
	def test_profile_switching(self):
		log = mock.Mock()
		local_config = self.get_default_local_config()
		local_config["ios"]["profiles"]["other_profile"] = {"developer_certificate": "dummy certificate"}

		config = build.ToolConfig(log, local_config, ['--profile', 'other_profile'], ['ios'])

		eq_(config["ios.profile.developer_certificate"], "dummy certificate")
	...
	def test_profile_switching_with_typo(self):
		...
		assert_raises(build.ArgumentError, lambda: build.ToolConfig(
			log, local_config, ['--profile', 'other_profile'], ['ios']))
	
	def test_no_DEFAULT_profile(self):
		log = mock.Mock()
		local_config = self.get_default_local_config()
		del local_config["ios"]["profiles"]["DEFAULT"]

		config = build.ToolConfig(log, local_config, [], ['ios'])
		eq_(config.get("ios.profile.developer_certificate"), None)
