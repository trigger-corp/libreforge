...
def enabled_platforms(build_type_dir):
    ...
    '''
    return ['reload', 'web', 'ios', 'android', 'ios-native', 'android-native']
...
class Build(object):
    ...
    def __init__(self, config, source_dir, output_dir, temp_dir=None,
                    system_config=None,
                    external=True, usercode=None, userassets=None, ignore_patterns=None,
                    enabled_platforms=('android', 'web'),
                    log=None, template_only=False, forge_root=None,
                    local_config=None, extra_args=None, override_modules=False, is_remote=True,
                    meta_config={}, **kw):
        ...
        :param temp_dir: directory to use for intermediate and temporary build files
        ...
        :param userassets: location of customer app assets that need to be integrated in the server-side build (e.g. iOS app icons)
        ...
        if config is not None and "android_sdk_root" in config:
            config["android_sdk_dir"] = os.path.join(config['android_sdk_root'], 'api-30')
            config.pop("android_sdk_root")
        self.config = config
        self.source_dir = path.abspath(source_dir)
        self.output_dir = path.abspath(output_dir)
        self.temp_dir = path.abspath(temp_dir) if temp_dir is not None else None
        self.external = external
        self.usercode = usercode if usercode else path.join(self.source_dir, 'user')
        self.userassets = userassets if userassets else path.join(self.source_dir, 'assets')
        ...
        if system_config is not None and "android_sdk_root" in system_config:
            system_config["android_sdk_dir"] = os.path.join(system_config['android_sdk_root'], 'api-30')
